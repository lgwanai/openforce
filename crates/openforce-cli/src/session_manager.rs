...
impl SessionManager {
    pub async fn new(workspace: PathBuf) -> Self {
        ...
        let backend = if !redis_url.is_empty() {
            match openforce_redis_session::store::RedisSessionStore::connect(&redis_url).await {
                ...
                Err(e) => {
                    tracing::warn!("Redis unavailable ({}), fallback to local", e);
                    Backend::Local
                }
            }
        } else {
            tracing::info!("Session store: local (.openforce/sessions/)");
            Backend::Local
        };
    ...
    pub async fn create(&mut self, goal: &str) -> Result<LocalSessionState, String> {
        match &mut self.backend {
            Backend::Redis(redis) => {
                let r = redis
                    .create(&self.workspace.display().to_string(), goal)
                    .await?;
                Ok(self.to_local(&r))
            }
            Backend::Local => {
    ...
    pub async fn resume(&mut self, id: Option<&str>) -> Result<LocalSessionState, String> {
        match &mut self.backend {
            Backend::Redis(redis) => {
                let r = match id {
                    Some(sid) => redis
                        .load(&Uuid::parse_str(sid).map_err(|e| format!("invalid: {e}"))?)
                        .await?
                        .ok_or("session not found")?,
                    None => {
                        let list = redis
                            .list(Some(&self.workspace.display().to_string()))
                            .await?;
    ...
    pub async fn approve(&mut self, id: Option<&str>) -> Result<LocalSessionState, String> {
        match &mut self.backend {
            Backend::Redis(redis) => {
                let s = match id {
                    ...
                    None => {
                        let list = redis
                            ...
                            .await?;
                        redis
                            .load(&list.first().ok_or("no sessions")?.session_id)
                            .await?
                            .ok_or("session not found")?
                    }
                };
                ...
                redis
                    .resolve_gate(
                        &openforce_redis_session::store::Caller::Planner {
