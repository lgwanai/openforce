...
impl AgentMemory {
    ...
    fn add_finding(&mut self, finding: &str) {
        ...
        if !self.key_findings.contains(&s) {
            self.key_findings.push(s);
        }
    }
...
fn parse_tool_args(
    ...
    let args: Value =
        serde_json::from_str(&call.arguments).map_err(|e| format!("parse args: {e}"))?;
...
impl ToolExecutor {
    fn resolve_read_path(&self, requested: &str) -> Result<PathBuf, String> {
        let matched = self
            ...
            .files_available
            .iter()
            .find(|f| f.ends_with(requested))
            .or_else(|| {
                let fname = Path::new(requested)
                    .file_name()
                    .and_then(|n| n.to_str())
                    .unwrap_or(requested);
                self.memory
                    .files_available
                    .iter()
                    .find(|f| f.ends_with(fname))
            })
            .cloned()
            .unwrap_or_else(|| requested.to_string());
        self.resolve_existing_path(&matched)
    }
    ...
    fn resolve_existing_path(&self, requested: &str) -> Result<PathBuf, String> {
        ...
        if !canonical.starts_with(&root) {
            return Err(format!("path outside workspace: {requested}"));
        }
        Ok(canonical)
    }

    fn resolve_write_path(&self, requested: &str) -> Result<PathBuf, String> {
        let path = Path::new(requested);
        if path
            .components()
            .any(|c| matches!(c, std::path::Component::ParentDir))
        {
            return Err(format!("path traversal not allowed: {requested}"));
        }
        let candidate = if path.is_absolute() {
            path.to_path_buf()
        } else {
        ...
        let canonical_parent = if parent.exists() {
            parent
                .canonicalize()
                .map_err(|e| format!("resolve parent {requested}: {e}"))?
        } else {
            let mut existing = parent;
            while !existing.exists() {
                existing = existing
                    .parent()
                    .ok_or_else(|| format!("cannot resolve parent for {requested}"))?;
            }
            existing
                .canonicalize()
                .map_err(|e| format!("resolve parent {requested}: {e}"))?
        ...
        if !canonical_parent.starts_with(&root) {
            return Err(format!("write outside workspace: {requested}"));
        }
        Ok(candidate)
    }
    ...
    async fn execute_read(&mut self, call: &ToolCall) -> ToolResult {
        let args = match parse_tool_args(call, &["path"]) {
            Ok(a) => a,
            Err(e) => return ToolResult::error(&call.id, &e),
        };
        let path = &args["path"];
        let matched = match self.resolve_read_path(path) {
            Ok(p) => p,
            Err(e) => return ToolResult::error(&call.id, &e),
        };
        match fs::read_to_string(&matched) {
            Ok(content) => {
                let truncated = if content.len() > 16000 {
                    format!(
                        "{}\n... (truncated, {} total)",
                        safe_slice(&content, 16000),
                        content.len()
                    )
                } else {
                    content.clone()
                };
                self.active_file_content = Some((matched.display().to_string(), truncated.clone()));
                self.memory.add_decision(
                    0,
                    "READ",
                    &format!("{} ({} chars)", matched.display(), content.len()),
                );
                ToolResult::success(&call.id, &truncated)
            }
            Err(e) => ToolResult::error(&call.id, &format!("read {}: {e}", matched.display())),
        }
    }

    fn execute_write(&mut self, call: &ToolCall) -> ToolResult {
        let args = match parse_tool_args(call, &["path", "content"]) {
            Ok(a) => a,
            Err(e) => return ToolResult::error(&call.id, &e),
        };
        let (path, content) = (&args["path"], &args["content"]);
        let target = match self.resolve_write_path(path) {
            Ok(p) => p,
            Err(e) => return ToolResult::error(&call.id, &e),
        };
        ...
        match fs::write(&target, content) {
            Ok(_) => {
                self.memory.add_decision(
                    0,
                    "WRITE",
                    &format!("{} ({} chars)", target.display(), content.len()),
                );
                ToolResult::success(
                    &call.id,
            ...
            Err(e) => ToolResult::error(&call.id, &format!("write {}: {e}", target.display())),
        }
    }

    async fn execute_shell(&mut self, call: &ToolCall) -> ToolResult {
        let args = match parse_tool_args(call, &["command"]) {
            Ok(a) => a,
            Err(e) => return ToolResult::error(&call.id, &e),
        };
        ...
        match output {
            Ok(Ok(out)) => {
                ...
                let truncated = if combined.len() > 4000 {
                    format!("{}\n... (truncated)", safe_slice(&combined, 4000))
                } else {
                    combined.clone()
                };
                self.active_file_content = Some((format!("SHELL:{cmd}"), truncated.clone()));
                self.memory
                    .add_decision(0, "SHELL", &format!("{cmd} ({} chars)", combined.len()));
                ToolResult::success(&call.id, &truncated)
            }
    ...
    fn execute_finding(&mut self, call: &ToolCall) -> ToolResult {
        let args = match parse_tool_args(call, &["text"]) {
            Ok(a) => a,
            Err(e) => return ToolResult::error(&call.id, &e),
        };
        let text = &args["text"];
    ...
    fn execute_ask_user(&mut self, call: &ToolCall) -> ToolResult {
        let args = match parse_tool_args(call, &["question"]) {
            Ok(a) => a,
            Err(e) => return ToolResult::error(&call.id, &e),
        };
        let q = &args["question"];
        let opts = serde_json::from_str::<serde_json::Value>(&call.arguments)
            .ok()
    ...
    fn execute_report_blocker(&mut self, call: &ToolCall) -> ToolResult {
        let args = match parse_tool_args(call, &["reason"]) {
            Ok(a) => a,
            Err(e) => return ToolResult::error(&call.id, &e),
        };
    ...
    fn execute_skill_load(&mut self, call: &ToolCall) -> ToolResult {
        let args = match parse_tool_args(call, &["name"]) {
            Ok(a) => a,
            Err(e) => return ToolResult::error(&call.id, &e),
        };
        let name = &args["name"];
        if let Some(body) = self.skill_registry.load_skill_body(name) {
            ...
            self.active_file_content =
                Some((format!("SKILL:{name}"), format!("{body}{dirs_info}")));
            ToolResult::success(
                &call.id,
                &format!("Skill {name} loaded ({} chars)", body.len()),
            )
        } else {
    ...
    fn execute_skill_ref(&mut self, call: &ToolCall) -> ToolResult {
        let args = match parse_tool_args(call, &["skill", "path"]) {
            Ok(a) => a,
            Err(e) => return ToolResult::error(&call.id, &e),
        };
        ...
        match executor.load_reference(sn, rp) {
            Some(content) => {
                ...
                ToolResult::success(
                    &call.id,
                    &format!("Reference {sn}/{rp} loaded ({} chars)", content.len()),
                )
            }
    ...
    async fn execute_skill_script(&mut self, call: &ToolCall) -> ToolResult {
        let args: Value = match serde_json::from_str(&call.arguments) {
            Ok(v) => v,
            Err(e) => return ToolResult::error(&call.id, &format!("parse args: {e}")),
        };
        let (sn, sp) = match (
        ...
        match executor.execute_script(sn, sp, &sargs).await {
            Ok(output) => {
                let truncated = if output.len() > 4000 {
                    format!("{}\n... (truncated)", safe_slice(&output, 4000))
                } else {
                    output.clone()
                };
...
impl ContextManager {
    ...
    fn context_limit(&self) -> usize {
        for (prefix, limit) in MODEL_LIMITS {
            if self.model.contains(prefix) {
                return *limit;
            }
        }
        128_000
...
async fn main() -> Result<()> {
    ...
    let api_key = task
        .api_key
        .as_deref()
        .unwrap_or(&std::env::var("API_KEY").unwrap_or_default())
        .to_string();
    let base_url = task
        .base_url
        .as_deref()
    ...
    let max_dur = Duration::from_secs(
        std::env::var("WORKER_MAX_SECS")
            .ok()
            .and_then(|s| s.parse().ok())
            .unwrap_or(1200),
    );
    let max_cycles: usize = std::env::var("WORKER_MAX_CYCLES")
        .ok()
        .and_then(|s| s.parse().ok())
        .unwrap_or(40);
    let _max_no_prog: usize = std::env::var("WORKER_MAX_NO_PROGRESS")
        .ok()
        .and_then(|s| s.parse().ok())
        .unwrap_or(8);
    let mut tokens_used: usize = 0;
    let max_tokens: usize = std::env::var("WORKER_MAX_TOKENS")
        .ok()
        .and_then(|s| s.parse().ok())
        .unwrap_or(2_000_000);
    ...
    if !skill_registry.is_empty() {
        eprintln!(
            "[Skills] {} available",
            skill_registry.enabled_skill_names().len()
        );
    }
    ...
    let ac: Vec<String> = plan["acceptance_criteria"]
        .as_array()
        .map(|a| {
            a.iter()
    ...
    let subs: Vec<SubTask> = plan["subtasks"]
        .as_array()
        .map(|a| {
            a.iter()
                .enumerate()
        ...
        .unwrap_or_else(|| {
            vec![SubTask {
                id: 1,
                description: task.subtask.clone(),
                status: "pending".into(),
                output: String::new(),
            }]
        });
    ...
    let initial_active_file_content = if let Some(ref bs_name) = task.bound_skill {
        skill_registry
            .load_skill_body(bs_name)
            .map(|body| (format!("SKILL:{bs_name}"), body))
    } else {
        None
    };
    let mut executor = ToolExecutor {
        memory,
        active_file_content: initial_active_file_content,
        skill_registry,
        workspace_root,
    };
    ...
    eprintln!(
        "[Agent] {} subtasks, {} criteria, {} files",
        executor.memory.subtasks.len(),
        executor.memory.acceptance_criteria.len(),
    ...
    'subtask_loop: for st_idx in 0..subtask_count {
        ...
        for st_cycle in 0..max_st {
            ...
            if start.elapsed() > max_dur {
                ...
                write_output(
                    &task,
                    &serde_json::json!({"success":d>0,"action":if d>0{"completed_partial"}else{"timeout"},"subtasks_done":d,"subtasks_total":subtask_count}),
                );
                return Ok(());
            }
            if tokens_used >= max_tokens {
                ...
                write_output(
                    &task,
                    &serde_json::json!({"success":d>0,"action":if d>0{"completed_partial"}else{"token_budget"},"subtasks_done":d,"subtasks_total":subtask_count}),
                );
                return Ok(());
            }
            ...
            if blocker_reasons.len() >= 3
                && (0..3).all(|i| {
                    blocker_reasons[blocker_reasons.len() - 1 - i]
                        == blocker_reasons[blocker_reasons.len() - 1]
                })
            {
                eprintln!(
                    "  BLOCKED subtask {}/{}: same failure 3x",
                    st_idx + 1,
                    subtask_count
                );
                break;
            ...
            let est_tokens: usize = conversation
                .iter()
                .map(|m| ContextManager::estimate_tokens(&m.content) + 100)
                .sum();
            if ctx_mgr.is_overflow(est_tokens) {
                eprintln!(
                    "  [Compaction] context {est_tokens}/{} tokens → compressing",
                    ctx_mgr.context_limit()
                );
                let history_text: String = conversation
                    .iter()
                    .map(|m| {
                        format!(
                            "[{}]: {}",
                            if m.role.contains("assistant") {
                                "ASSISTANT"
                            } else {
                                "USER"
                            },
                ...
                match perform_compaction(&client, &system, &history_text).await {
                    Ok(summary) => {
                        conversation.clear();
                        conversation.push(openforce_llm_client::unified::ToolMessage::user(
                            &format!("[CONTEXT COMPACTED — preserved below]\n\n{summary}",),
                        ));
            ...
            let todo_status: String = executor
                .memory
                .subtasks
                .iter()
                .enumerate()
                .map(|(i, s)| {
                    let icon = if i < st_idx {
                        "✓"
                    } else if i == st_idx {
                        "▶"
                    } else {
            ...
            match client.chat_with_tools(&system, &conversation, &tools).await {
                Ok(response) => {
                    ...
                    if response.has_tool_calls() {
                        conversation.push(
                            openforce_llm_client::unified::ToolMessage::assistant_with_tools(
                                &response.text,
                                &response.tool_calls,
                            ),
                        );
                            ...
                            if tc.name == "compress_context" {
                                ...
                                match perform_compaction(&client, &system, &history_text).await {
                                    Ok(summary) => {
                                        executor
                                            .memory
                                            .add_finding(&format!("[compacted] {summary}"));
                                        tr.push(ToolResult::success(
                                            &tc.id,
                                            &format!("Compacted context summary:\n\n{summary}"),
                                        ));
                                    }
                                    Err(e) => {
                                        tr.push(ToolResult::error(
                                            &tc.id,
                        ...
                        conversation.push(
                            openforce_llm_client::unified::ToolMessage::tool_results(&tr),
                        );
                        continue;
                    }
                    ...
                    if (upper.contains("VERIFIED")
                        && !upper.contains("NOT VERIFIED")
                        && !upper.contains("UNVERIFIED"))
                        || upper.contains("FINAL: PASS")
                    {
                        ...
                        executor
                            .memory
                            .add_decision(cycles, "WORKING", &truncate_str(text, 120));
                    }
                }
                Err(e) => {
                    ...
                    write_output(
                        &task,
                        &serde_json::json!({"success":false,"action":"error","detail":format!("{e}")}),
                    );
                    std::process::exit(1);
                }
            }
        ...
        executor.memory.subtasks[st_idx].output =
            format!("BLOCKERS: {}", blocker_reasons.join(" | "));
    }
    // Phase 2: Verification
    let vp = format!(
        "{}\n\nFINAL: rate each criterion PASS/FAIL. End with: FINAL: PASS|FAIL",
        build_context_string(&executor)
    );
    match client.chat(&system, &vp).await {
        Ok((text, _)) => {
            ...
            write_output(
                &task,
                &build_worker_output(&executor, &task, cycles, tokens_used, &text, passed),
            );
        }
        Err(e) => {
            write_output(
                &task,
                &serde_json::json!({"success":false,"action":"error","reason":"verification_failed","detail":format!("{e}")}),
            );
        }
    }
    Ok(())
