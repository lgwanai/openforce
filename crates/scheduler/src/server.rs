...
use openforce_proto::swarmos::v1::{
    scheduler_server::Scheduler as SchedulerTrait, session_store_client::SessionStoreClient,
    CancelTaskRequest, CancelTaskResponse, Command as ProtoCommand, CompilePlanRequest,
    CompilePlanResponse, ExecuteCommandRequest, ExecuteCommandResponse, LeaseTaskRequest,
    LeaseTaskResponse, MarkTaskSucceededRequest, MarkTaskSucceededResponse,
    MarkTaskTimedOutRequest, MarkTaskTimedOutResponse, RenewLeaseRequest, RenewLeaseResponse,
    ReplanSessionRequest, ReplanSessionResponse, SendHeartbeatRequest, SendHeartbeatResponse,
    SubmitArtifactRequest, SubmitArtifactResponse, SubmitFindingRequest, SubmitFindingResponse,
...
impl SchedulerService {
    pub fn new(
        ...
        instance_id: String,
        token_issuer: Option<Arc<CapabilityTokenIssuer>>,
    ) -> Self {
        Self {
            session_store_addr,
            instance_id,
    ...
    async fn get_client(&self) -> Result<SessionStoreClient<tonic::transport::Channel>, Status> {
        ...
        if guard.is_none() {
            ...
            let client = SessionStoreClient::connect(addr)
                .await
...
impl SchedulerTrait for SchedulerService {
    async fn compile_plan(
        &self,
        r: Request<CompilePlanRequest>,
    ) -> Result<Response<CompilePlanResponse>, Status> {
        let cmd = r
            .into_inner()
            .command
    ...
    async fn submit_artifact(
        &self,
        r: Request<SubmitArtifactRequest>,
    ) -> Result<Response<SubmitArtifactResponse>, Status> {
        let cmd = r
            .into_inner()
            .command
            .ok_or(Status::invalid_argument("command required"))?;
    ...
    async fn submit_patch(
        &self,
        r: Request<SubmitPatchRequest>,
    ) -> Result<Response<SubmitPatchResponse>, Status> {
        let cmd = r
            .into_inner()
            .command
            .ok_or(Status::invalid_argument("command required"))?;
    ...
    async fn submit_finding(
        &self,
        r: Request<SubmitFindingRequest>,
    ) -> Result<Response<SubmitFindingResponse>, Status> {
        let cmd = r
            .into_inner()
            .command
            .ok_or(Status::invalid_argument("command required"))?;
    ...
    async fn mark_task_succeeded(
        &self,
        r: Request<MarkTaskSucceededRequest>,
    ) -> Result<Response<MarkTaskSucceededResponse>, Status> {
        let cmd = r
            .into_inner()
            .command
            .ok_or(Status::invalid_argument("command required"))?;
    ...
    async fn mark_task_timed_out(
        &self,
        r: Request<MarkTaskTimedOutRequest>,
    ) -> Result<Response<MarkTaskTimedOutResponse>, Status> {
        let cmd = r
            .into_inner()
            .command
            .ok_or(Status::invalid_argument("command required"))?;
    ...
    async fn replan_session(
        &self,
        r: Request<ReplanSessionRequest>,
    ) -> Result<Response<ReplanSessionResponse>, Status> {
        let cmd = r
            .into_inner()
            .command
            .ok_or(Status::invalid_argument("command required"))?;
    ...
    async fn send_heartbeat(
        &self,
        r: Request<SendHeartbeatRequest>,
        ...
        if tenant_id.is_empty() || session_id.is_empty() || task_id.is_empty() {
            return Err(Status::invalid_argument(
                "tenant_id, session_id, and task_id are required",
            ));
        }
        if fencing_token == 0 {
            return Err(Status::invalid_argument(
                "fencing_token is required for heartbeat",
            ));
        }
        ...
        let cmd = ProtoCommand {
            ...
            task_id,
            expected_version: 0,
            payload: serde_json::to_vec(&serde_json::json!({"fencing_token": fencing_token}))
                .unwrap_or_default(),
            ..Default::default()
        };
