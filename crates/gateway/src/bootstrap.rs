...
impl App {
    pub fn new(addr: SocketAddr) -> Self {
        ...
        Self {
            addr,
            session_store_addr: std::env::var("SESSION_STORE_ADDR")
                .unwrap_or_else(|_| "127.0.0.1:50051".into()),
            project_tools_addr: std::env::var("PROJECT_TOOLS_ADDR")
                .unwrap_or_else(|_| "127.0.0.1:50053".into()),
            effect_gateway_addr: std::env::var("EFFECT_GATEWAY_ADDR")
        ...
        match std::env::var("SCHEDULER_PUBLIC_KEY_PATH") {
            Ok(path) => match std::fs::read(&path) {
                Ok(bytes) => {
                    if bytes.len() != 32 {
                        tracing::error!(
                            "SCHEDULER_PUBLIC_KEY_PATH={path}: expected 32 bytes, got {}",
                            bytes.len()
                        );
                        None
                    } else {
    ...
    pub fn router(&self) -> Router {
        ...
        let mut router = Router::new()
            .nest(
                ...
                handler::project_tools::routes().layer(axum::middleware::from_fn(
                    middleware::require_capability_token,
                )),
            )
            .nest(
                "/api/v1/approvals",
                handler::approvals::routes().layer(axum::middleware::from_fn(
                    middleware::require_capability_token,
                )),
            )
            .nest(
                "/api/v1/effects",
                handler::effects::routes().layer(axum::middleware::from_fn(
                    middleware::require_capability_token,
                )),
            )
