...
impl ImageManager {
    ...
    pub async fn ensure_image(&self, image: &SandboxImage) -> SandboxResult<CachedImage> {
        ...
        if kp.exists() && rp.exists() {
            let c = CachedImage {
                kernel_path: kp,
                rootfs_path: rp,
                digest: image.image_digest.clone(),
            };
            self.cache
                .write()
                .await
                .insert(image.image_digest.clone(), c.clone());
            return Ok(c);
        }
    ...
    pub fn verify_digest(path: &std::path::Path, expected: &str) -> SandboxResult<()> {
        let data =
            std::fs::read(path).map_err(|e| crate::error::SandboxError::IoError { source: e })?;
        let mut h = Sha256::new();
        h.update(&data);
        let actual = format!("sha256:{:x}", h.finalize());
        if actual != expected {
            return Err(crate::error::SandboxError::ImageDigestMismatch {
