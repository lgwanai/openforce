...
use openforce_domain::approval::{ApprovalRequest, ApprovalStatus, ApprovalToken};
...
impl ApprovalStore {
    ...
    fn load_or_generate_signing_key() -> Ed25519KeyPair {
        ...
        if let Ok(path) = std::env::var("APPROVAL_SIGNING_KEY_PATH") {
            ...
            tracing::warn!(
                "APPROVAL_SIGNING_KEY_PATH not set — approval signing key will not survive restart"
            );
        }
    ...
    pub async fn approve_request(
        ...
        .ok_or(DomainError::ValidationFailed {
            detail: "approval request not found".into(),
        })?;
        ...
        .bind(req.session_id)
        .bind(req.task_id)
        .bind(req.task_attempt)
        .bind(req.lease_id)
        .bind(req.fencing_token)
        .bind(req.worker_spec_id)
        .bind(&req.tool_name)
        .bind(&req.target_paths)
        .bind(&req.base_snapshot_id)
        .bind(&req.payload_sha256)
        .bind(ApprovalStatus::Approved.as_str())
        .bind(usage_limit as i32)
        .bind(0i32)
        .bind(now)
        .bind(expires_at)
        .bind(approver_id)
        ...
        Ok(ApprovalToken {
            ...
            tool_name: req.tool_name,
            target_paths: serde_json::from_value(req.target_paths).unwrap_or_default(),
            base_snapshot_id: req.base_snapshot_id,
            payload_sha256: req.payload_sha256,
    ...
    pub async fn consume_token(
        ...
        let token = ApprovalToken {
            ...
            task_attempt: token.task_attempt,
            ...
            base_snapshot_id: token.base_snapshot_id,
            payload_sha256: token.payload_sha256,
            status: ApprovalStatus::Approved,
            usage_limit: token.usage_limit as u32,
