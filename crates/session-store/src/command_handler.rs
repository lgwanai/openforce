...
use crate::repo::ProjectionRepo;
use crate::store::EventStore;
use chrono::Duration;
use openforce_domain::command::{Command, CommandResult, CommandType};
use openforce_domain::error::{DomainError, DomainResult};
use openforce_domain::event::{DagEdgeDef, EventEnvelope, EventPayload, ProducerIdentity, TaskDef};
use openforce_domain::lease;
...
impl CmdHandler {
    pub async fn execute(
        ...
        if let Some(r) = Self::check_dedup(pool, cmd.command_id).await? {
            return Ok(r);
        }
    ...
    async fn check_dedup(pool: &PgPool, cid: Uuid) -> DomainResult<Option<CommandResult>> {
        let row: Option<(serde_json::Value,)> =
            sqlx::query_as("SELECT result FROM command_dedup WHERE command_id = $1")
                .bind(cid)
                .fetch_optional(pool)
                .await
        ...
        if let Some((json,)) = row {
            ...
            return Ok(Some(CommandResult {
                command_id: cid,
                success: true,
                new_session_version: obj
                    .get("new_session_version")
                    .and_then(|v| v.as_i64())
                    .unwrap_or(0),
    ...
    fn parse_plan_payload(
    ...
    ) -> DomainResult<(Vec<TaskDef>, Vec<DagEdgeDef>)> {
        if payload.is_null() {
            return Ok((vec![], vec![]));
        }
        ...
        struct PlanPayload {
            ...
            tasks: Vec<TaskDef>,
            #[serde(default)]
            dag_edges: Vec<DagEdgeDef>,
        }
        let parsed: PlanPayload =
            serde_json::from_value(payload.clone()).map_err(|e| DomainError::ValidationFailed {
                detail: format!("invalid CompilePlan payload: {e}"),
            })?;
        for task in &parsed.tasks {
            Uuid::parse_str(&task.task_id).map_err(|e| DomainError::ValidationFailed {
                detail: format!("invalid task_id '{}': {e}", task.task_id),
            })?;
        }
        for edge in &parsed.dag_edges {
            Uuid::parse_str(&edge.from_task_id).map_err(|e| DomainError::ValidationFailed {
                detail: format!("invalid from_task_id '{}': {e}", edge.from_task_id),
            })?;
            Uuid::parse_str(&edge.to_task_id).map_err(|e| DomainError::ValidationFailed {
                detail: format!("invalid to_task_id '{}': {e}", edge.to_task_id),
            })?;
        }
        Ok((parsed.tasks, parsed.dag_edges))
    }

    async fn compile_plan(pool: &PgPool, cmd: &Command) -> DomainResult<CommandResult> {
        let ver = ProjectionRepo::get_session(pool, cmd.session_id)
            .await
            .map(|s| s.current_plan_version)
            .unwrap_or(0);
        let nv = ver + 1;
        let (tasks, dag_edges) = Self::parse_plan_payload(&cmd.payload)?;
        let evts = vec![
            EventEnvelope::new(
                "PlanEpochStarted",
                cmd.session_id,
        ...
        let new_ver =
            EventStore::append_events(pool, cmd.session_id, cmd.expected_version, &evts).await?;
        ProjectionBuilder::rebuild(pool, cmd.session_id).await?;
        Ok(Self::res(
            cmd.command_id,
            new_ver,
            serde_json::json!({"plan_version": nv, "plan_epoch": nv}),
        ))
    }

    async fn mark_task_ready(pool: &PgPool, cmd: &Command) -> DomainResult<CommandResult> {
        let tid = cmd.task_id.ok_or(DomainError::ValidationFailed {
            detail: "task_id required".into(),
        })?;
        let t = ProjectionRepo::get_task(pool, cmd.session_id, tid).await?;
        if t.state != TaskState::Pending {
            return Err(DomainError::TaskNotReady);
        }
        let evt = EventEnvelope::new(
            ...
            cmd.tenant_id,
            cmd.requested_by.clone(),
            EventPayload::TaskReadied(openforce_domain::event::TaskReadiedPayload {}),
        )
        .with_task(tid, t.task_attempt, 0, t.plan_epoch);
        let new_ver =
            EventStore::append_events(pool, cmd.session_id, cmd.expected_version, &[evt]).await?;
        ProjectionBuilder::rebuild(pool, cmd.session_id).await?;
        Ok(Self::res(
            cmd.command_id,
            new_ver,
            serde_json::json!({"task_id": tid.to_string(), "state": "Ready"}),
        ))
    }
    ...
    async fn lease_task(
        ...
        let na = t.task_attempt + 1;
        let ft = t.current_fencing_token + 1;
        let l = lease::Lease::new(
            ...
            ft,
            Uuid::now_v7(),
            Duration::seconds(lease::DEFAULT_LEASE_TTL_SECS),
            lease::DEFAULT_HEARTBEAT_INTERVAL_SECS,
        );
        let evt = EventEnvelope::new(
            "TaskLeased",
            cmd.session_id,
            ...
            EventPayload::TaskLeased(openforce_domain::event::TaskLeasedPayload {
                ...
                renewal_deadline: l
                    ...
                    .unwrap_or_default(),
                fencing_token: ft,
                worker_spec_id: l.worker_spec_id.to_string(),
    ...
    async fn submit_artifact(pool: &PgPool, cmd: &Command) -> DomainResult<CommandResult> {
        ...
        let fencing_token = cmd
            .payload
            .get("fencing_token")
            .and_then(|v| v.as_u64())
            .ok_or(DomainError::ValidationFailed {
                detail: "fencing_token required in payload".into(),
            })?;
        ...
        let evt = EventEnvelope::new(
            ...
            EventPayload::ArtifactSubmitted(openforce_domain::event::ArtifactSubmittedPayload {
                ...
                artifact_type: "generic".into(),
                storage_uri: String::new(),
                content_sha256: String::new(),
                produced_by_spec: String::new(),
        ...
        Ok(Self::res(
            cmd.command_id,
            new_ver,
            serde_json::json!({"task_id": tid.to_string()}),
        ))
    }
    ...
    async fn submit_finding(pool: &PgPool, cmd: &Command) -> DomainResult<CommandResult> {
        ...
        let evt = EventEnvelope::new(
            ...
            EventPayload::FindingSubmitted(openforce_domain::event::FindingSubmittedPayload {
                finding_type: "generic".into(),
                finding_data: serde_json::json!({}),
            }),
        )
        .with_task(tid, t.task_attempt, 0, t.plan_epoch);
        let new_ver =
    ...
    async fn request_effect(pool: &PgPool, cmd: &Command) -> DomainResult<CommandResult> {
        let tid = cmd.task_id.ok_or(DomainError::ValidationFailed {
            detail: "task_id required for effect request".into(),
        })?;
        let t = ProjectionRepo::get_task(pool, cmd.session_id, tid).await?;
        t.verify_submissible()?;
        let fencing_token = cmd
            ...
            .and_then(|v| v.as_u64())
            .ok_or(DomainError::ValidationFailed {
                detail: "fencing_token required in payload for effect request".into(),
            })?;
        t.verify_fencing(fencing_token)?;
        let effect_type = cmd
            .payload
        ...
        Ok(Self::res(
            cmd.command_id,
            new_ver,
            serde_json::json!({"status": "requested", "task_id": tid.to_string()}),
        ))
    }
    ...
    async fn mark_succeeded(pool: &PgPool, cmd: &Command) -> DomainResult<CommandResult> {
        ...
        Ok(Self::res(
            cmd.command_id,
            new_ver,
            serde_json::json!({"task_id": tid.to_string(), "state": "Succeeded"}),
        ))
    }
    ...
    async fn mark_timed_out(pool: &PgPool, cmd: &Command) -> DomainResult<CommandResult> {
        ...
        Ok(Self::res(
            cmd.command_id,
            new_ver,
            serde_json::json!({"task_id": tid.to_string(), "state": "TimedOut"}),
        ))
    }
    ...
    async fn replan(pool: &PgPool, cmd: &Command) -> DomainResult<CommandResult> {
        ...
        let nv = s.current_plan_version + 1;
        let ne = s.current_plan_epoch + 1;
        let (tasks, dag_edges) = Self::parse_plan_payload(&cmd.payload)?;
        let evts = vec![
            EventEnvelope::new(
                "PlanEpochStarted",
        ...
        Ok(Self::res(
            cmd.command_id,
            new_ver,
            serde_json::json!({"new_plan_version": nv, "new_plan_epoch": ne}),
        ))
    }
    ...
    async fn cancel(pool: &PgPool, cmd: &Command) -> DomainResult<CommandResult> {
        ...
        Ok(Self::res(
            cmd.command_id,
            new_ver,
            serde_json::json!({"task_id": tid.to_string(), "state": "Cancelled"}),
        ))
    }
}
