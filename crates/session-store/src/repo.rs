...
use openforce_domain::task::{ReplanDisposition, Task, TaskState};
use sqlx::PgPool;
use uuid::Uuid;
...
impl ProjectionRepo {
    pub async fn get_session(pool: &PgPool, session_id: Uuid) -> DomainResult<Session> {
        ...
        row.map(|r| Session {
            ...
            current_plan_version: r.current_plan_version,
            current_plan_epoch: r.current_plan_epoch,
    ...
    pub async fn get_task(pool: &PgPool, session_id: Uuid, task_id: Uuid) -> DomainResult<Task> {
        ...
        row.map(|r| {
            ...
            let downstream: Vec<Uuid> =
                serde_json::from_value(r.downstream_ids).unwrap_or_default();
            Task {
                session_id: r.session_id,
                task_id: r.task_id,
    ...
    pub async fn list_tasks(
        ...
        rows.into_iter()
            .map(|r| {
                let upstream: Vec<Uuid> =
                    serde_json::from_value(r.dependency_ids).unwrap_or_default();
                let downstream: Vec<Uuid> =
                    serde_json::from_value(r.downstream_ids).unwrap_or_default();
                Ok(Task {
                    session_id: r.session_id,
...
struct SessionRow {
    ...
    goal: String,
    state: String,
    current_plan_version: i32,
    current_plan_epoch: i32,
    session_version: i64,
...
struct TaskRow {
    ...
    state: String,
    task_attempt: i32,
    current_lease_id: Option<Uuid>,
    current_fencing_token: i64,
    current_worker_spec_id: Option<Uuid>,
    plan_epoch: i32,
    replan_disposition: String,
    dependency_ids: serde_json::Value,
    downstream_ids: serde_json::Value,
    updated_at: chrono::DateTime<chrono::Utc>,
}
