use crate::command_handler::CmdHandler;
...
use openforce_proto::swarmos::v1::{
    session_store_server::SessionStore as SessionStoreTrait, AppendEventsRequest,
    AppendEventsResponse, ExecuteCommandRequest, ExecuteCommandResponse, GetSessionRequest,
    GetSessionResponse, GetTaskRequest, GetTaskResponse, ListTasksRequest, ListTasksResponse,
    ReadEventLogRequest, RebuildProjectionRequest, RebuildProjectionResponse,
};
use sqlx::PgPool;
...
fn de(e: openforce_domain::error::DomainError) -> Status {
    ...
    match &e {
        ...
        | DomainError::TenantNotFound { .. } => Status::not_found(e.to_string()),
...
impl SessionStoreTrait for SessionStoreService {
    async fn append_events(
        &self,
        r: Request<AppendEventsRequest>,
    ) -> Result<Response<AppendEventsResponse>, Status> {
        let req = r.into_inner();
        let sid = pu(&req.session_id)?;
        let tid = pu(&req.tenant_id)?;
        let events: Vec<openforce_domain::event::EventEnvelope> = req
            .events
            .into_iter()
            .map(|e| {
                ...
                openforce_domain::event::EventEnvelope {
                    ...
                    session_version: 0,
                    payload: serde_json::from_slice(&e.payload).unwrap_or(
                        openforce_domain::event::EventPayload::TaskReadied(
            ...
            .collect();
        let nv = EventStore::append_events(&self.pool, sid, req.expected_version, &events)
            .await
            .map_err(de)?;
        Ok(Response::new(AppendEventsResponse {
            new_session_version: nv,
        }))
    }

    async fn execute_command(
        &self,
        r: Request<ExecuteCommandRequest>,
        ...
        let dc = DomainCommand {
            command_id: pu(&c.command_id)?,
            command_type: ct,
            tenant_id: pu(&c.tenant_id)?,
            session_id: pu(&c.session_id)?,
            task_id: if c.task_id.is_empty() {
                None
            } else {
                pu(&c.task_id).ok()
            },
            expected_version: c.expected_version,
            requested_by: prod(self),
            requested_at: chrono::Utc::now(),
            payload: serde_json::from_slice(&c.payload).unwrap_or(serde_json::Value::Null),
        };
        let res = CmdHandler::execute(&self.pool, dc, prod(self))
            .await
            .map_err(de)?;
        let rj = serde_json::to_vec(&res.result).map_err(|e| Status::internal(e.to_string()))?;
        Ok(Response::new(ExecuteCommandResponse {
            result: rj,
            new_session_version: res.new_session_version,
        }))
    }

    async fn get_session(
        &self,
        r: Request<GetSessionRequest>,
    ) -> Result<Response<GetSessionResponse>, Status> {
        let sid = pu(&r.into_inner().session_id)?;
        let s = ProjectionRepo::get_session(&self.pool, sid)
            .await
            .map_err(de)?;
        Ok(Response::new(GetSessionResponse {
            ...
            current_plan_version: s.current_plan_version,
            current_plan_epoch: s.current_plan_epoch,
    ...
    async fn get_task(
        &self,
        r: Request<GetTaskRequest>,
    ) -> Result<Response<GetTaskResponse>, Status> {
        let req = r.into_inner();
        let t = ProjectionRepo::get_task(&self.pool, pu(&req.session_id)?, pu(&req.task_id)?)
            .await
            .map_err(de)?;
        Ok(Response::new(GetTaskResponse {
            ...
            current_worker_spec_id: t
                .current_worker_spec_id
                .map(|id| id.to_string())
                .unwrap_or_default(),
            plan_epoch: t.plan_epoch,
            replan_disposition: t.replan_disposition.as_str().into(),
            upstream_task_ids: t
                .upstream_task_ids
                .iter()
                .map(|id| id.to_string())
                .collect(),
            downstream_task_ids: t
                .downstream_task_ids
                .iter()
                .map(|id| id.to_string())
                .collect(),
    ...
    async fn list_tasks(
        &self,
        r: Request<ListTasksRequest>,
    ) -> Result<Response<ListTasksResponse>, Status> {
        let req = r.into_inner();
        let sf = if req.state_filter.is_empty() {
            None
        } else {
            Some(req.state_filter)
        };
        let tasks = ProjectionRepo::list_tasks(&self.pool, pu(&req.session_id)?, sf)
            .await
            .map_err(de)?;
        let pts: Vec<GetTaskResponse> = tasks
            .into_iter()
            .map(|t| GetTaskResponse {
                session_id: t.session_id.to_string(),
                task_id: t.task_id.to_string(),
                task_type: t.task_type,
                state: t.state.as_str().into(),
    ...
    type ReadEventLogStream =
        tokio_stream::wrappers::ReceiverStream<Result<openforce_proto::swarmos::v1::Event, Status>>;

    async fn read_event_log(
        &self,
        r: Request<ReadEventLogRequest>,
    ) -> Result<Response<Self::ReadEventLogStream>, Status> {
        let req = r.into_inner();
        let events = EventStore::read_event_log(
            &self.pool,
            pu(&req.session_id)?,
            req.from_session_version,
            req.max_events,
        )
        .await
        .map_err(de)?;
        ...
        for e in events {
            let _ = tx
                .send(Ok(openforce_proto::swarmos::v1::Event {
                    ...
                    producer: Some(openforce_proto::swarmos::v1::ProducerIdentity {
                        component: e.producer.component,
                        instance_id: e.producer.instance_id,
    ...
    async fn rebuild_projection(
        &self,
        r: Request<RebuildProjectionRequest>,
    ) -> Result<Response<RebuildProjectionResponse>, Status> {
        let sid = pu(&r.into_inner().session_id)?;
        ProjectionBuilder::rebuild(&self.pool, sid)
            .await
            .map_err(de)?;
        let s = ProjectionRepo::get_session(&self.pool, sid)
            .await
            .map_err(de)?;
        Ok(Response::new(RebuildProjectionResponse {
            success: true,
            rebuilt_to_version: s.session_version,
        }))
    }
}
