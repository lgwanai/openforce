...
impl EventStore {
    pub async fn append_events(
        ...
        // pg_advisory_xact_lock(int, int) takes 4-byte ints, so cast from i64 → i32
        sqlx::query("SELECT pg_advisory_xact_lock($1::int, $2::int)")
            .bind(lock_key.0 as i32)
            .bind(lock_key.1 as i32)
            .execute(&mut *tx)
            .await
            .map_err(|e| DomainError::ValidationFailed {
        ...
        for event in events {
            ...
            .bind(event.plan_version)
            .bind(event.plan_epoch)
            .bind(event.task_id)
            .bind(event.task_attempt)
            .bind(event.causation_id)
            .bind(event.correlation_id)
            .bind(&event.producer.component)
            .bind(&event.producer.instance_id)
            .bind(&event.producer.region)
            .bind(next)
            .bind(serde_json::to_value(&event.payload).map_err(|e| {
...
impl EventLogRow {
    fn to_envelope(self) -> DomainResult<EventEnvelope> {
        let payload: openforce_domain::event::EventPayload =
            serde_json::from_value(self.payload).map_err(|e| DomainError::ValidationFailed {
                detail: format!("deser: {e}"),
            })?;
        Ok(EventEnvelope {
            ...
            plan_version: self.plan_version as i64,
            plan_epoch: self.plan_epoch,
            task_id: self.task_id,
            task_attempt: self.task_attempt.unwrap_or(0),
            producer: openforce_domain::event::ProducerIdentity {
                component: self.producer_component,
                instance_id: self.producer_instance,
