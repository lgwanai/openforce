...
mod tests {
    ...
    fn test_worker_status_from_action() {
        assert_eq!(WorkerStatus::from_status_and_action(true, "completed"), WorkerStatus::Completed);
        assert_eq!(WorkerStatus::from_status_and_action(false, "blocked"), WorkerStatus::Blocked);
        assert_eq!(WorkerStatus::from_status_and_action(false, "stalled"), WorkerStatus::Stalled);
        assert_eq!(WorkerStatus::from_status_and_action(false, "timeout"), WorkerStatus::Stalled);
        assert_eq!(WorkerStatus::from_status_and_action(false, "error"), WorkerStatus::Failed);
    }
    ...
    fn test_format_for_prompt_mixed() {
        let folders = vec![
            WorkerOutputFolder {
                worker_id: "w0".into(), role: "dev".into(), title: "task1".into(),
                status: WorkerStatus::Completed, one_liner: "All tests pass".into(),
                key_notes: vec!["Coverage at 85%".into()],
                output_ref: OutputRef::file("/tmp/x.json"),
                subtasks_done: 5, subtasks_total: 5, cycles: 3,
            },
            WorkerOutputFolder {
                worker_id: "w1".into(), role: "tester".into(), title: "task2".into(),
                status: WorkerStatus::Failed, one_liner: "Tests failed".into(),
                key_notes: vec!["3/5 tests fail".into(), "Null pointer in handler".into()],
                output_ref: OutputRef::file("/tmp/y.json"),
                subtasks_done: 2, subtasks_total: 5, cycles: 2,
            },
        ];
    ...
    fn test_format_for_prompt_completed_no_findings() {
        let folders = vec![WorkerOutputFolder {
            worker_id: "w0".into(), role: "dev".into(), title: "t".into(),
            status: WorkerStatus::Completed, one_liner: "Done".into(),
            key_notes: vec![], output_ref: OutputRef::file("/tmp/x.json"),
            subtasks_done: 1, subtasks_total: 1, cycles: 1,
        }];
        let result = WorkerOutputFolder::format_for_prompt(&folders);
    ...
    fn test_serialization_roundtrip() {
        let f = WorkerOutputFolder {
            worker_id: "w1".into(), role: "architect".into(), title: "Design API".into(),
            status: WorkerStatus::Completed, one_liner: "API designed with 3 endpoints".into(),
            key_notes: vec!["RESTful".into(), "OpenAPI 3.0".into()],
            output_ref: OutputRef::redis("sid", "w1"),
            subtasks_done: 3, subtasks_total: 3, cycles: 5,
        };
