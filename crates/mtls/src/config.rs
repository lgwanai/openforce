...
fn load_cert_chain(pem: &[u8]) -> MTLSResult<Vec<CertificateDer<'static>>> {
    ...
    rustls_pemfile::certs(&mut reader)
        ...
        .map_err(|e| MTLSError::TlsConfigError {
            detail: e.to_string(),
        })
}
...
fn load_private_key(pem: &[u8]) -> MTLSResult<PrivateKeyDer<'static>> {
    ...
    let key = rustls_pemfile::private_key(&mut reader)
        ...
        .ok_or_else(|| MTLSError::TlsConfigError {
...
fn load_ca_store(pem: &[u8]) -> MTLSResult<RootCertStore> {
    ...
    for ca in ca_certs {
        store.add(ca).map_err(|e| MTLSError::TlsConfigError {
            detail: e.to_string(),
        })?;
    }
    Ok(store)
...
pub fn load_bundle_from_env(role: &str, instance_id: &str) -> Option<CertificateBundle> {
    let cert_pem = std::env::var("SWARMOS_TLS_CERT").ok().or_else(|| {
        std::fs::read_to_string(format!(
            "{}/{}/cert.pem",
            std::env::var("SWARMOS_TLS_DIR").unwrap_or_default(),
            role
        ))
        .ok()
    })?;
    let key_pem = std::env::var("SWARMOS_TLS_KEY").ok().or_else(|| {
        std::fs::read_to_string(format!(
            "{}/{}/key.pem",
            std::env::var("SWARMOS_TLS_DIR").unwrap_or_default(),
            role
    ...
    let ca_cert_pem = std::env::var("SWARMOS_CA_CERT").ok().or_else(|| {
        std::fs::read_to_string(format!(
            "{}/ca.pem",
            std::env::var("SWARMOS_TLS_DIR").unwrap_or_default()
        ))
    ...
    Some(CertificateBundle {
        ...
        ca_cert_pem: ca_cert_pem.into_bytes(),
        role,
...
impl TlsConfigBuilder {
    pub fn server_config_with_mutual_tls(bundle: &CertificateBundle) -> MTLSResult<ServerConfig> {
        let _ = default_provider()
            .install_default()
            .map_err(|_| MTLSError::TlsConfigError {
                detail: "crypto provider init failed".into(),
            })?;
