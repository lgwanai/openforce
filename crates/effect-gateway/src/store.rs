...
impl EffectStore {
    ...
    pub async fn request_effect(
        ...
        .bind(payload_ref)
        .bind(EffectState::Requested.as_str())
        .bind(now)
        .bind(now)
        .fetch_one(&self.pool)
        .await
        .map_err(|e| DomainError::ValidationFailed {
            detail: format!("insert effect: {e}"),
        })?;
    ...
    pub async fn approve_effect(
        ...
        .bind(effect_id)
        .bind(EffectState::Approved.as_str())
        .bind(approved_by)
        .bind(now)
        .execute(&mut *tx)
        .await
        ...
        if rows.rows_affected() == 0 {
            ...
            return Err(DomainError::ValidationFailed {
                detail: "effect not in requested state".into(),
            });
        }
        sqlx::query(
    ...
    pub async fn commit_effect(
        ...
        .bind(effect_id)
        .bind(EffectState::Committed.as_str())
        .bind(execution_ref)
        .bind(now)
        .execute(&self.pool)
        .await
    ...
    pub async fn get_effect(
        ...
        row.ok_or(DomainError::ValidationFailed {
            detail: "effect not found".into(),
        })
    }
}
