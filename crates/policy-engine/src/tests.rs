...
mod tests {
    ...
    #[test]
    fn test_scheduler_lease_allowed() {
        let engine = PolicyEngine::with_defaults();
        let ctx = AuthzContext {
            mTLS_identity: Some(ct_identity(ServiceRole::scheduler())),
            capability_token: None,
            requested_action: "LeaseTask".into(),
    ...
    #[test]
    fn test_worker_with_token_allowed() {
        let tid = Uuid::now_v7();
        let sid = Uuid::now_v7();
        ...
        let ctx = AuthzContext {
            ...
            capability_token: Some(test_token(
                tid,
                sid,
                tid2,
                lid,
                5,
                vec![TokenScope::artifact_submit()],
            )),
            requested_action: "SubmitArtifact".into(),
            resource_type: "Artifact".into(),
            tenant_id: Some(tid),
            session_id: Some(sid),
            task_id: Some(tid2),
            lease_id: Some(lid),
            current_fencing_token: Some(5),
        };
        assert!(matches!(engine.evaluate(&ctx), PolicyEffect::Allow { .. }));
    }

    #[test]
    fn test_worker_without_token_denied() {
        let engine = PolicyEngine::with_defaults();
        let ctx = AuthzContext {
            mTLS_identity: Some(ct_identity(ServiceRole::worker())),
            capability_token: None,
    ...
    #[test]
    fn test_stale_fencing_token_denied() {
        let tid = Uuid::now_v7();
        let sid = Uuid::now_v7();
        let tid2 = Uuid::now_v7();
        let lid = Uuid::now_v7();
    ...
    #[test]
    fn test_node_daemon_spawn_allowed() {
        let engine = PolicyEngine::with_defaults();
        let ctx = AuthzContext {
            mTLS_identity: Some(ct_identity(ServiceRole::node_daemon())),
            capability_token: None,
            requested_action: "SpawnWorker".into(),
    ...
    #[test]
    fn test_wrong_role_for_action_denied() {
        let engine = PolicyEngine::with_defaults();
        let ctx = AuthzContext {
            mTLS_identity: Some(ct_identity(ServiceRole::worker())),
            capability_token: None,
    ...
    #[test]
    fn test_custom_always_allow_rule() {
        let custom = PolicyRule::new(
            "always",
            200,
    ...
    #[test]
    fn test_empty_rules_returns_deny() {
        let engine = PolicyEngine::new(vec![]);
        let ctx = AuthzContext {
            mTLS_identity: None,
            capability_token: None,
            requested_action: "any".into(),
    ...
    #[test]
    fn test_equal_priority_first_match_wins() {
        let allow_first = PolicyRule::new(
            "allow",
            100,
            RuleCondition::Always,
            PolicyEffect::Allow {
        ...
        let deny_second = PolicyRule::new(
            "deny",
            100,
            RuleCondition::Always,
            PolicyEffect::Deny {
