...
impl SandboxManager {
    pub fn new(
        ...
        Self {
            vm_manager,
            image_manager,
            credential_injector: CredentialInjector::new(metadata_dir),
        }
    }
    ...
    pub async fn inject_credentials(
        ...
        let tc: Vec<TempCredential> = creds
            .iter()
            .map(|c| TempCredential {
                key: c.key.clone(),
                value: c.value.clone(),
        ...
        self.credential_injector
            .inject(&sandbox_id.to_string(), &tc)
