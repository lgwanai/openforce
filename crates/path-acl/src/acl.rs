...
impl PathAcl {
    ...
    pub fn can_read(&self, path: &str) -> bool {
        if !self.forbidden.is_empty() && self.matches(&self.forbidden, path) { return false; }
        self.allowed_read.is_empty() || self.matches(&self.allowed_read, path)
    }

    pub fn can_write(&self, path: &str) -> bool {
        if !self.forbidden.is_empty() && self.matches(&self.forbidden, path) { return false; }
        self.allowed_write.is_empty() || self.matches(&self.allowed_write, path)
    }
