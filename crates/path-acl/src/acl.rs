...
impl PathAcl {
    ...
    pub fn can_read(&self, path: &str) -> bool {
        ...
        if !self.forbidden.is_empty() && self.matches(&self.forbidden, &n) {
            return false;
        }
...
fn compile(paths: &[String]) -> Vec<Pattern> {
    paths
        ...
        .filter_map(|p| {
            Pattern::new(p)
                .map_err(|e| {
                    tracing::warn!("Invalid ACL pattern '{}': {}", p, e);
                })
                .ok()
        })
        .collect()
...
mod tests {
    ...
    fn test_normalize_dot() {
        assert_eq!(normalize_path("a/./b"), Some("a/b".into()));
    }
    #[test]
    ...
    #[test]
    fn test_deep_traversal_blocked() {
        assert_eq!(normalize_path("a/../../etc"), None);
    }

    #[test]
    fn test_traversal_normalized_into_forbidden() {
    ...
    #[test]
    fn test_traversal_cannot_escape_allowed_dir() {
