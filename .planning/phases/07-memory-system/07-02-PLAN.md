...
Purpose: Manage session context with compression and retrieval.
Output: ShortTermMemory class with sliding window and summarization.
</objective>

<tasks>

<task type="auto">
  <name>Create short_term.py</name>
  <files>src/memory/short_term.py</files>
  <action>
    ...
    class ShortTermMemory:
        ...
        def _create_summary(self, messages: List[MemoryItem]) -> SessionSummary:
            ...
            summary_text = f"Session segment with {len(messages)} messages. "
            summary_text += "Key topics: " + "; ".join(key_points[:5])
        ...
        def from_json(cls, json_str: str) -> 'ShortTermMemory':
            ...
            memory = cls()
            memory._messages = [MemoryItem.from_dict(m) for m in data.get("messages", [])]
...
<task type="auto" tdd="true">
  ...
  <files>tests/memory/test_short_term.py</files>
</task>
...
- [ ] ShortTermMemory with sliding window
